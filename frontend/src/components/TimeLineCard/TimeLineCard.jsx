...
export default function TimeLineCard({ events }) {
  ...
  const importantEvents = timeline.filter((event) => [0, 2, 3, 5, 71].includes(event.Type));
            ...
            <Card key={event.EventId} variant="outlined">
              <CardContent>
                <Stack direction="row" spacing={1} alignItems="center">
                  <Typography variant="h6">{getIcon(event.Type)}</Typography>
                  <Chip label={event.MatchMinute} size="small" />
