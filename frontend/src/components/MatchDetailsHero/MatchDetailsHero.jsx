...
function buildCardGradient(colors) {
  return `linear-gradient(135deg, ${colors.secondary} 0%, ${colors.secondary}cc 50%, ${colors.primary} 100%)`;
}
...
export default function MatchDetailsHero({ match }) {
  ...
  const scoreLabel = matchHasScore && Number.isFinite(match.homeScore) && Number.isFinite(match.awayScore)
  ...
  return (
    ...
    >
      <CardContent sx={{ pt: 4, pb: 4, px: { xs: 2, md: 4 } }}>
        <Stack direction="row" justifyContent="center" alignItems="center" spacing={3} mb={{ xs: 5, md: 7 }}>
          ...
          <Chip
            label={status.label}
            size="small"
            sx={{ fontWeight: 700, bgcolor: status.background, color: status.color }}
          />
        </Stack>
        ...
        >
          <Box sx={{ width: "100%", display: "flex", justifyContent: "center" }}>
            <Stack alignItems="center" justifyContent="flex-start" spacing={1} sx={{ minHeight: { xs: 95, md: 120 } }}>
              <TeamBlock flag={match.homeFlag} name={match.homeTeam} />
              <TeamName name={match.homeTeam} />
            </Stack>
          </Box>

          <Box sx={{ textAlign: "center", minWidth: { xs: 90, sm: 120, md: 280 } }}>
            <Typography
              variant="body2"
              sx={{
                alignSelf: "center",
